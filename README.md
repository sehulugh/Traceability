...
![alt text](https://github.com/sehulugh/Traceability/blob/master/pix.png?raw=true)
...
5. JOIN the result in 4. with **AppTransactionLogs** to get the **transactionDate, UseraName, Action, ModelName** _and_ _model_ **Key(s) and KeyValue(s)** (from substringing ModelKeyValues)
6. Again, JOIN resulting table from 5. with **vMetaDataTables** to get the _ **ValueNames** of **KeyValues** _ in 5 where exists
7. Finally JOIN resulting table in 6. above with **vRelatedKeys** in order to get _ **ParentKey** and **ParentValue** for keys that are children. Eg if key is **ContactId**, it is a child to **VendorId** so we need to relate it to it&#39;s **VendorId** 
